...
class CrowDbHelper(context: Context) :
    ...
    override fun onCreate(db: SQLiteDatabase) {
        db.execSQL(
            ...
            CREATE TABLE $TABLE_VIDEOS (
                ...
                $COL_SEEK_JUMP_SEC INTEGER NOT NULL DEFAULT 15,
    ...
    /**
     * Inserts a new row or updates only scan-derived columns so playback prefs and thumbnails stay intact.
     */
    fun insertOrMergeFromScan(entity: VideoEntity): Long {
        ...
        if (existingId == null) {
            ...
            return db.insert(TABLE_VIDEOS, null, cv)
        }
    ...
    fun getById(id: Long): VideoEntity? {
        readableDatabase.query(
            ...
            if (c.moveToFirst()) return c.toEntity()
        }
        return null
    }
